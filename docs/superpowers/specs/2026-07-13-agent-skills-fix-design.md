...
- ผลลัพธ์เป็น **metadata ฉบับสุดท้าย** — ทับทั้งของ script agent และของที่ critic ปรับมา (critic คืน metadata ได้เช่นกัน; เมื่อ flag เปิด metadata agent ชนะเสมอ)
- script prompt_template คงเดิม (ยังออก title/desc/tags) — เป็น fallback ตอน flag ปิดหรือ metadata agent ล้มเหลว
...
2. **Frequency gate:** top issue เดิม (นับจาก `TopIssues` ที่ learner aggregate จาก critique reasons อยู่แล้ว) ปรากฏใน ≥40% ของ critiques ในหน้าต่าง
