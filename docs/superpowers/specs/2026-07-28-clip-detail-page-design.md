...
> หมายเหตุหลังรีวิว: "ถูกกัก" = `auto_review_held` เป็นจริง **และ** สถานะเป็น
> `needs_review` หรือ `ready` เท่านั้น — คลิปที่ auto-review เพิ่งกักยังคงเป็น
> `needs_review` เพราะ `SetAutoReviewHeld` พลิกแค่ธง ไม่แตะสถานะ (ตรงกับ guard
> ของ `ClearAutoReviewHeld`) ถ้าเช็คแค่ `ready` ปุ่มจะกลายเป็น "อนุมัติ" ที่ตั้ง
> สถานะเป็น ready โดยธงยังค้าง แล้ว publisher ข้ามคลิปนั้นไปตลอด:
