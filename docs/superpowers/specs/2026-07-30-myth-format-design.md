...
**ผลที่ตามมา (แก้ 2026-07-30 หลัง final review):** คลังพร้อมใช้จริง **4 แถว** ไม่ใช่ 6 —
แถว "เพจไลก์เยอะ = แอดถูกลง" และ "Boost post = ยิงแอด" ถูกปรับเป็น `needs_verify=TRUE`
เพราะข้อเท็จจริงถูกแต่ `source_label` ที่ร่างไว้ตอนแรก ("ผลสำรวจ organic reach 2024",
"คู่มือ Ads Manager 2026") **ไม่ใช่เอกสารที่มีอยู่จริง** — ชื่อแหล่งอ้างขึ้นจอในฐานะ
ค่าที่เชื่อถือได้ ปล่อยไว้เท่ากับฟีเจอร์ที่สร้างมากันการแต่งข้อเท็จจริงกลับส่งการอ้างอิง
ที่แต่งขึ้นออกจอเอง · ตะแกรงเพิ่ม `source_label <> ''` ในระดับ SQL แล้ว (ดู §7.5)
→ ดู §9 เงื่อนไขก่อนเปิด schedule
...
5. **ตะแกรงระดับ SQL** (เพิ่ม 2026-07-30): `mythAvailableWhere` บังคับ `source_label <> ''`
   ทั้งในตัวเลือก ตัว fallback และตัวนับพื้นคลัง — เส้นทางขยายคลังใน §9 คือ "พลิก
   `needs_verify` เป็น FALSE" ถ้าใครพลิกโดยลืมเติมแหล่งอ้าง คลิปจะขึ้นจอว่า
   "แหล่งอ้าง:" เปล่าๆ โดยไม่มีคนตรวจ ตะแกรงนี้กันแทนความจำของคน
6. **ตะแกรงครอบ metadata** (เพิ่ม 2026-07-30): title/description ของ YouTube ผ่าน
   `mythGateFailure` ด้วย — สเปกเดิม §7.2 เขียนแค่ narration + on-screen text แต่สองอันนี้
   เผยแพร่จริงและเป็นสิ่งแรกที่คนอ่าน
7. **แถวที่ตกตะแกรงถูกนับใช้+พัก** (เพิ่ม 2026-07-30): ไม่งั้นมันยังเป็นแถวที่ใช้น้อยสุด
   พรุ่งนี้ ⇒ ช่อง 09:00 ได้คลิปค้าง `needs_review` ทุกวันและแถวอื่นไม่ได้คิว (ช่องตายเงียบ
   แบบมีคลิปแต่ไม่เผยแพร่) แยกจาก error ระบบด้วย `ErrContentGateBlocked`
...
**เงื่อนไขก่อนเปิด schedule** (คลังพร้อมใช้ **4 แถว**, cooldown 14 วัน):
- 4 แถว = เปิดได้สัปดาห์ละไม่เกิน 2 วัน (`0 9 * * 1,4`) โดยไม่ซ้ำหัวข้อภายใน 14 วัน
- ถึง 6-8 แถว → `0 9 * * 1,3,5` (จ/พ/ศ)
- ถึง **14 แถวที่ `needs_verify=false` และมี `source_label`** → `0 9 * * *` ทุกวัน
- การพลิก `needs_verify` ต้องเติม `fact_th` ที่ยืนยันได้ + `source_label` ที่เป็นเอกสารจริง
  (ตะแกรง SQL ใน §7.5 กันแถวไร้แหล่งอ้างไว้แล้ว แต่ไม่กันข้อความ `fact_th` ที่ยัง TODO)
