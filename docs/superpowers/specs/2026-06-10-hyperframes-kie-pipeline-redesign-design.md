...
- **Gemini** — `POST https://api.kie.ai/gemini/v1/models/gemini-3-5-flash:streamGenerateContent`, Google format. Body: `{contents:[{role:"user",parts:[{text}]}], tools:[{googleSearch:{}}]?, generationConfig:{temperature}}`. **Returns Server-Sent Events** (confirmed by live call 2026-06-10): `data: {json}` lines terminated by `data: [DONE]` — split lines, strip `data:`, skip `[DONE]`/non-JSON, concatenate `candidates[].content.parts[].text`. Roles are `user`/`model` (not `assistant`). `generationConfig.temperature` and `googleSearch` both verified accepted (HTTP 200).
