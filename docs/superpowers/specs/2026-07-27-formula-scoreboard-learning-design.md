...
## 14. สิ่งที่เปลี่ยนไปหลังลงมือทำจริง (บันทึกวันที่ 2026-07-27 หลัง implement + review ครบ)

เอกสารข้างบนคือเจตนา ส่วนนี้คือสิ่งที่ต่างออกไปจริง — อ่านส่วนนี้ก่อนเชื่อรายละเอียดข้างบน

**อัลกอริทึม clamp (แทนที่หัวข้อ 6):** แผนเดิมใช้ลูป clamp/normalize 3 รอบตายตัว ซึ่งพิสูจน์แล้วว่า
ไม่การันตีเพดาน (ทดสอบได้ค่า 0.52 เทียบเพดาน 0.50 และที่ alpha 0.3-0.5 ทะลุเป็นจำนวนเต็มจริง)
โค้ดจริงใช้ **water-filling**: ตรึงค่าที่ชนขอบ ตัดออกจากงบ แล้วคำนวณที่เหลือใหม่จนไม่มีใครหลุด
มีขอบเขตรอบไม่เกินจำนวนสูตร ผลรวมเป็น 100 ในทางปฏิบัติ แต่ property test 20,000 เคสพบ 25 เคส
ที่ได้ 101-102 — ไม่กระทบพฤติกรรมเพราะตัวเลือกสูตรใช้อัตราส่วน `used/weight` ไม่ใช่ค่าสัมบูรณ์

**การนับปัญหาซ้ำ (แทนที่หัวข้อ 8 ข้อ 2):** ต้องนับด้วย `COUNT(DISTINCT cc.id)` ไม่ใช่ `COUNT(*)`
critique ใบเดียวแก้ `scene[0]`, `scene[1]`, `scene[2].voice_text` ต้องนับเป็น 1 ใบ เพราะเกณฑ์
เทียบกับ "จำนวนใบ critique" วัดกับ prod: 39 จาก 83 ใบ = 0.47 เทียบเกณฑ์ 0.40

**ระยะพักของ learner (เพิ่มใหม่ ไม่มีในแผน):** ประตูทั้งสามเป็น "ระดับ" ไม่ใช่ "เหตุการณ์" — สภาพ
0.47 ค้างอยู่หลายสัปดาห์และการเขียน skills ใหม่ไม่ทำให้มันลดลงในรอบเดียว ถ้าไม่กัน learner จะ
เขียนทับ skills ทุกวันจันทร์ตลอดไป จึงเพิ่ม **ระยะพัก 28 วันต่อ agent** อ่านจาก `skill_revisions`

**outcome gate (แทนที่หัวข้อ 8 ข้อ 3):** ต้องแบ่งช่วงด้วย `clips.created_at` ไม่ใช่ `fetched_at`
(ระบบดึงสถิติทุกคลิปทุกวัน หน้าต่างตาม fetched_at จึงมีคลิปชุดเดียวกัน) และจัดอันดับบนชุดอ้างอิงร่วม
90 วัน ไม่ใช่แยกหน้าต่าง (ไม่งั้น "ต่ำกว่า P25 ของชุดตัวเอง" ได้ ~25% เสมอโดยนิยาม)
วัดกับ prod หลังแก้: window 0.305 vs baseline 0.241 · **ข้อจำกัดที่รู้ตัว:** จัดอันดับด้วยยอดวิวดิบ
ซึ่งสะสมตามอายุ คลิปใหม่จึงเสียเปรียบเชิงโครงสร้าง ส่วนต่าง 0.064 ที่วัดได้จึงเป็นสัญญาณจริงเพียงบางส่วน

**Rollback (เพิ่มจากหัวข้อ 9):** ต้องบันทึก revision ของตัวเอง (สลับ old/new) ไม่งั้นตารางประวัติ
จะยังยืนยันว่าน้ำหนักที่ปรับไปยังใช้อยู่ และยึดกฎ audit-ก่อน-apply เหมือนเส้นทางปกติ

**เพิ่มในลำดับปล่อย (หัวข้อ 12):** ขั้นที่ 1 (`POST /api/v1/formula-scores/compute`) **ห้ามข้าม** —
ส่วน analyzer ไม่ได้อยู่หลัง flag มันเปลี่ยนไปกินกระดานคะแนนทันทีที่ deploy ถ้ายังไม่มี snapshot
โมเดลจะได้ข้อมูล **น้อยกว่า** เดิม (คลิปหัวท้าย 10 ตัว โดยไม่มีกระดานคะแนน) โดยไม่มีอะไรเตือน

**คำถามที่ยังค้าง ต้องตัดสินก่อนเปิดสวิตช์:** query ใหม่ยุบเป็นแถวเดียวต่อ (คลิป, แพลตฟอร์ม) โดยทิ้ง
`post_type` ตามแบบ `analyzer.gatherData` เดิม แต่ YouTube มีทั้ง `shorts` (140 คลิป) และ `regular`
(53 คลิป) และ **42 คลิปมีทั้งคู่** — ยอดวิวของคลิปหนึ่งจึงสลับไปมาระหว่างสองแถวได้ระหว่าง snapshot

**เพิ่ม `maturityDays = 14` (2026-07-27 หลัง deploy รอบแรก):** กระดานคะแนนนับเฉพาะคลิปที่อายุเกิน 14 วัน
วัดจากข้อมูลจริง 52 คลิป YouTube ที่อายุเกิน 21 วัน — คลิปได้ **43% ของวิวทั้งหมดใน 7 วันแรก และ 89% ใน 14 วัน**
คลิปที่ใหม่กว่านั้นยังสะสมวิวไม่จบ พอเอาไปจัดอันดับปนกับคลิปที่นิ่งแล้วจึงกดคะแนนของสูตรที่บังเอิญมีคลิปใหม่เยอะ
ก่อนแก้: `qa` มีคลิปอายุน้อยกว่า 14 วันแค่ 15% ได้อันดับหนึ่ง ส่วน `case_story` มี 41% ได้อันดับสุดท้าย
หลังแก้ n เหลือ 12-65 ต่อสูตร (ยังเกินเกณฑ์ 8) และ `qa` ยังนำ **แถมนำห่างขึ้น** (0.53 → 0.59 บน YouTube)
แปลว่าความได้เปรียบของ qa ไม่ได้มาจากอายุล้วนๆ ส่วน `case_story` รั้งท้ายทั้งสองแพลตฟอร์มจริง

**ผลข้างเคียงที่ตั้งใจ:** 3 หมวดที่ enabled อยู่ (`account-buyer`, `grey-operator`, `performance-advertiser`)
เพิ่งเริ่มใช้ 17-18 ก.ค. อายุเฉลี่ย ~4.5 วัน จึงถูกกรองออกเกือบหมด → มิติ `category` จะถูกตรึง (n<8)
จนถึงราวกลาง ส.ค. ซึ่งเป็นพฤติกรรมที่ถูกต้อง: ยังไม่มีข้อมูลพอก็ไม่ควรหมุน
